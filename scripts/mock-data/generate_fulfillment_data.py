    ...
    for x in range(0, random.randint(500, 1000)):
        record = {
            'payload': {
                ...
                'accountId': '{}-{}'.format(id, x),
                'creationTimestamp': creationTimestamp,
                'success': bool(random.choice([0, 1], p=[0.005, 0.995])),
            },
            'metadata': {
...
def write_avro_records(id, records):
    fpath = 'fulfillment-{}.avro'.format(id)
...
for x in range(0, 200000):
    ...
    fpath = delayed(upload_to_s3)(BUCKET, fpath, 'fulfillment-data')
